...
@api.get("/get-all-query/", response_model=List[Query], tags=["(Admin) Roles"])
def get_all_queries(query_svc: QueryService = Depends(QueryService)) -> List[Query]:
    return query_svc.get_all()


@api.post("/create-query", response_model=Query, tags=["(Admin) Roles"])
def create_query(
...
@api.delete("/delete-query/{query_id}", response_model=Query, tags=["(Admin) Roles"])
def delete_query(
